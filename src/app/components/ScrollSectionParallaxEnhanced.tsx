...
export default function ScrollSectionParallax() {
  return (
    <div className="content-stretch flex flex-col items-center justify-center relative w-full bg-white" style={{ zIndex: 1 }} data-name="Scroll Section Parallax">
      <div className="pt-16 md:pt-[120px] px-6 md:px-16 lg:px-[120px] w-full text-center">
        <p className="font-['Inter:Bold',sans-serif] font-bold leading-[normal] not-italic text-[#626262] text-[24px] md:text-[32px] whitespace-pre-wrap">
      ...
      <div className="md:hidden w-full px-4 py-10">
        <div className="grid grid-cols-2 gap-3">
          {[imgScrollSectionImage1Content, imgScrollSectionImage2Content, imgScrollSectionImage3, imgScrollSectionImage4Content, imgImage9, imgWhatsAppImage20260131At123837Pm1].map((src, i) => (
            <motion.div
              key={i}
              className={`rounded-xl overflow-hidden shadow-sm ${i === 0 ? 'row-span-2' : ''}`}
            ...
            >
              <img
                alt=""
                className={`w-full object-cover ${i === 0 ? 'h-full' : 'aspect-square'}`}
                src={src}
              />
            </motion.div>
          ))}
